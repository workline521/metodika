...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,100..900;1,14..32,100..900&family=Mulish:ital,wght@0,200..1000;1,200..1000&display=swap"
      rel="stylesheet" />
	<?php echo '<link rel="icon" type="image/x-icon" href="' . get_template_directory_uri() . '/assets/img/favicon.png">'; ?>
...
<header>
	<div class="container">
		...
		<?php echo '<a href="/" class="header__logo">
		<img src="' . get_template_directory_uri() . '/assets/img/logo.png" alt="pic"></a>'; ?>
			
			<div class="header__wrapper">				
				<?php
				$settings_page_id = 97; // ID страницы настроек контента хэдера
				$phone = get_field('phone', $settings_page_id);
				$work_hours = get_field('work_hours', $settings_page_id);
				$social_wa = get_field('social_wa', $settings_page_id);
				$social_tg = get_field('social_tg', $settings_page_id);
				$btn_text = get_field('consult_btn_text', $settings_page_id) ?: 'Бесплатная консультация';
				$rating = get_field('rating_value', $settings_page_id);
				?>													
				<div class="header__info">
					<?php if ( $social_wa || $social_tg ) : ?>
						<div class="header__social">
							<?php if ( $social_wa ) : ?>
								<a href="<?php echo esc_url($social_wa); ?>" target="_blank" rel="noopener" class="header__social-link social-wa">
					...
					<div class="header__contacts">
						...
						<?php endif; ?>
						<?php if ( $work_hours ) : ?>
							<div class="header__hours"><?php echo esc_html($work_hours); ?></div>
						<?php endif; ?>
					</div>						
				</div>
				<div class="header__consult-btn"> 
					<button class="btn btn--red" data-modal="consult"><?php echo esc_html($btn_text); ?>
					</button>
				</div>
				<button class="header__burger" aria-controls="primary-menu" aria-expanded="false">
					<svg class="burger-icon" xmlns="http://www.w3.org/2000/svg" width="26" height="18" viewBox="0 0 26 18" fill="none">
						<path class="burger-line line-1" d="M1.5 1.5H24" stroke="#EAF1F6" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
						<path class="burger-line line-2" d="M1.5 9.00003H24" stroke="#EAF1F6" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
						<path class="burger-line line-3" d="M1.5 16.5H24" stroke="#EAF1F6" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
					</svg>
		...
		<?php if ( $rating ) :
			...
			// Если дробная часть >= 0.75 – округляем до целой звезды
			if ($fraction >= 0.75) {
				$full_stars = ceil($rating);
				$half_star = false;
			} 
			// Иначе если дробная часть >= 0.5 – ставим половинку
			elseif ($fraction >= 0.5) {
				$half_star = true;
			}
			// Иначе полных звёзд уже достаточно (дробная часть < 0.5)

			// Корректируем, чтобы не вышло больше 5 звёзд
			if ($full_stars > 5) {
				$full_stars = 5;
				$half_star = false;
			}
			...
			if ($empty_stars < 0) $empty_stars = 0; // на всякий случай
		?>
